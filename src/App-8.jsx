      ...
      <div className="body-col">
        <div className="body-top">
          ...
          <span className="badge-group">
            ...
            {showDriverControls && (
              <span className="inline-driver-controls desktop-only" onClick={(e) => e.stopPropagation()}>
                {onAssignDriver && (
                  <select value={task.driver || ""} onChange={(e) => onAssignDriver(e.target.value)}>
                    <option value="">— ללא שיוך —</option>
                    {driverNames && driverNames.map((n) => <option key={n} value={n}>{n}</option>)}
                  </select>
                )}
                {onRemoveFromDay && <button className="remove-day-btn" onClick={onRemoveFromDay} aria-label="הסר מהיום" title="הסר מהיום"><X size={13}/></button>}
              </span>
            )}
          </span>
        </div>
...
.driver-assign-row { display:flex; align-items:center; gap:6px; margin-bottom: 10px; }
.driver-assign-row select { max-width: 170px; font-size:12px; padding:5px 8px; border-radius:999px; }
.driver-assign-row .btn-ghost.small { padding:5px 10px; font-size:11.5px; }
