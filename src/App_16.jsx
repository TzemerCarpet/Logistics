...
function ConfirmBlock({ task, onUpdate }) {
  ...
  const setStatus = (val) => onUpdate({ ...task, confirmStatus: val });
  const saveNote = () => { if (note !== (task.confirmNote || "")) onUpdate({ ...task, confirmNote: note }); };
  return (
    <div className="confirm-block" onClick={(e) => e.stopPropagation()}>
      <select className="confirm-status-select" value={task.confirmStatus || ""} onChange={(e) => setStatus(e.target.value)} style={task.confirmStatus ? { color: CONFIRM_COLORS[task.confirmStatus], borderColor: CONFIRM_COLORS[task.confirmStatus], fontWeight: 700 } : undefined}>
        <option value="">סטטוס שיחה</option>
        {CONFIRM_OPTIONS.map((opt) => <option key={opt} value={opt}>{opt}</option>)}
      </select>
      <button type="button" className={"confirm-note-toggle" + (task.confirmNote ? " has-note" : "")} title="הערה חופשית" onClick={() => setNoteOpen((v) => !v)}>📝</button>
      {noteOpen && (
        <input className="confirm-note-input" autoFocus placeholder="מה ההערה?" value={note} onChange={(e) => setNote(e.target.value)} onBlur={() => { saveNote(); setNoteOpen(false); }} onKeyDown={(e) => e.key === "Enter" && e.currentTarget.blur()} />
      )}
    </div>
  );
}
...
.confirm-status-select { border:1px solid #DED5BF; border-radius:999px; padding:2px 7px; font-size:11px; background:#FAFAF9; color:#3F3D38; max-width:96px; height:24px; }
.confirm-note-toggle { border:1px solid #DED5BF; border-radius:50%; width:24px; height:24px; background:#FAFAF9; font-size:12px; line-height:1; cursor:pointer; flex-shrink:0; padding:0; }
.confirm-note-toggle.has-note { border-color:#C77A2A; background:#FBF0DD; }
.confirm-note-input { position:absolute; left:0; top:28px; z-index:6; border:1px solid #DED5BF; border-radius:8px; padding:5px 9px; font-size:12px; background:#fff; box-shadow:0 4px 12px rgba(0,0,0,.12); width:170px; }
...
.group-driver-select { font-size:11.5px; font-weight:700; padding:4px 10px; border-radius:999px; border:none; background:#15161A; color:#fff; cursor:pointer; max-width:160px; height:26px; }
.group-driver-select:hover { background:#2B4A5E; }
.task-driver-inline { display:flex; align-items:center; justify-content:flex-end; gap:5px; margin-top:4px; }
.task-driver-inline select { border:1px solid #DED5BF; border-radius:999px; padding:2px 7px; font-size:11px; background:#FAFAF9; color:#3F3D38; max-width:110px; height:24px; }
...
@media (max-width: 560px) {
  ...
  .task-driver-inline .remove-day-btn, .inline-driver-controls .remove-day-btn { min-width:24px; min-height:24px; width:24px; height:24px; }
}
@media print {
