...
import {
  ...
  Wrench, Sparkles, MessageCircle, Building2, GripVertical, Search, ChevronUp, ChevronDown, CheckCircle2, Menu, Eye, EyeOff, Check
} from "lucide-react";
      ...
      <div className="modal">
        ...
        <div className="modal-body">
          ...
          {isBranch ? (
            ...
            <>
              ...
              <div className="form-grid">
                ...
                <Field label="קומה (לא חובה)"><input value={client.floor || ""} onChange={(e) => setClient({ ...client, floor: e.target.value })} placeholder="3" /></Field>
                <Field label="דירה (לא חובה)"><input value={client.apartment || ""} onChange={(e) => setClient({ ...client, apartment: e.target.value })} placeholder="12" /></Field>
                <Field label="מי נתן את המשימה"><select value={assignedBy} onChange={(e) => setAssignedBy(e.target.value)}><option value="">— בחר —</option>{assignerOptions.map((n) => <option key={n} value={n}>{n}</option>)}</select></Field>
              </div>
          ...
          <div className="form-grid">
            ...
            {!isBranch && <Field label="שעה שסוכמה עם הלקוח (לא חובה)"><input value={timeWindow} onChange={(e) => setTimeWindow(e.target.value)} placeholder="לדוגמה: 14:00-15:00" /></Field>}
          </div>
          ...
          {isCarpetType && (
            <>
              ...
              <Field label="מספר הזמנה ב-Priority (לא חובה)"><input value={priorityNumber} onChange={(e) => setPriorityNumber(e.target.value)} placeholder="לדוגמה: 12345" /></Field>
        ...
        <div className="modal-body">
          ...
          {!isGeneral && !isBranch && (
            <>
              ...
              <Field label="מספר הזמנה ב-Priority"><input value={priorityNumberDraft} onChange={(e) => setPriorityNumberDraft(e.target.value)} onBlur={savePriorityNumber} placeholder="לדוגמה: 12345" /></Field>
...
function OpenItemsTab({ tasks, updateTasks, account, accounts, driverNames }) {
  ...
  const filtered = useMemo(() => visible.filter((t) => OPEN_STATUSES.includes(t.status) && !t.pending && !t.rejected)
    .sort((a,b) => new Date(taskDateKey(b)) - new Date(taskDateKey(a))), [visible]);
...
function CalendarTab({ tasks, updateTasks, selectedDate, onSelectDate, onStartAssign, config, account, accounts, driverNames, getNextNumber }) {
  ...
  const dragRef = useRef(null); // { groupKey, taskId } — נשמר ב-ref כדי לא לגרום לרינדור בכל תזוזה
  const [draggingId, setDraggingId] = useState(null); // הדגשה ויזואלית של הנגרר
  const [overId, setOverId] = useState(null); // מעל איזו שורה נמצא הנגרר כרגע (לא משנה את הנתונים בפועל)
  const onHandlePointerDown = (e, groupKey, taskId) => {
    e.preventDefault();
    try { e.currentTarget.setPointerCapture(e.pointerId); } catch (err) {}
    dragRef.current = { groupKey, taskId };
  ...
  const onHandlePointerMove = (e) => {
    ...
    const row = el && el.closest && el.closest(".drag-row");
    if (!row) return;
    const targetGroup = row.dataset.group;
    const targetId = row.dataset.taskid;
    if (targetGroup !== dragRef.current.groupKey) return;
    setOverId((prev) => (prev === targetId ? prev : targetId));
  };
  const onHandlePointerUp = () => {
    ...
    setDraggingId(null);
    setOverId(null);
    if (!drag || !target || target === drag.taskId) return;
    const group = driverGroups.find((g) => g.key === drag.groupKey);
    if (!group) return;
    const arr = [...group.tasks];
    const fromIdx = arr.findIndex((t) => t.id === drag.taskId);
    const toIdx = arr.findIndex((t) => t.id === target);
  ...
  const reorderStop = (groupTasksArr, fromIdx, toIdx) => {
          ...
          return (
            <div key={group.key || "__unassigned__"} className="zone-block">
              ...
              <div className="task-list route-thread">
                {gTasks.map((t, i) => (
                  <div key={t.id} className={"drag-row" + (draggingId === t.id ? " dragging" : "") + (overId === t.id && draggingId !== t.id ? " drag-over" : "")} data-group={group.key} data-taskid={t.id}>
                    <div className="drag-handle" style={{ touchAction: "none" }} onPointerDown={(e) => onHandlePointerDown(e, group.key, t.id)} onPointerMove={onHandlePointerMove} onPointerUp={onHandlePointerUp} onPointerCancel={onHandlePointerUp}><GripVertical size={18} className="muted"/></div>
                    <div className="stop-position-controls">
                      <button type="button" disabled={i === 0} onClick={() => reorderStop(gTasks, i, i - 1)} aria-label="הזז למעלה"><ChevronUp size={14}/></button>
                      <input
                        type="number" className="stop-position-input" value={i + 1} min={1} max={gTasks.length}
                        onChange={(e) => {
                          const n = parseInt(e.target.value, 10);
                          if (!n || n < 1 || n > gTasks.length) return;
                          reorderStop(gTasks, i, n - 1);
                        }}
                      />
                      <button type="button" disabled={i === gTasks.length - 1} onClick={() => reorderStop(gTasks, i, i + 1)} aria-label="הזז למטה"><ChevronDown size={14}/></button>
                    </div>
                    <div style={{ flex: 1 }}>
                      <TaskCard
                        task={t}
    ...
    <div className={"driver-card" + (isTaskDone(task) ? " done" : "")}>
      ...
      {showHomeModal && (
        <div className="modal-backdrop" onClick={() => { saveHomeDetails(); setShowHomeModal(false); }}>
          <div className="modal" style={{ maxWidth: 340 }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <Field label="דירה"><input autoFocus value={aptDraft} onChange={(e) => setAptDraft(e.target.value)} placeholder="לדוגמה: 12" /></Field>
              <Field label="קומה"><input value={floorDraft} onChange={(e) => setFloorDraft(e.target.value)} placeholder="לדוגמה: 3" /></Field>
      ...
      {!isBranch && (
        task.driverNotDone ? (
          ...
          <div className="not-done-row">
            <input placeholder="למה לא בוצע? (חובה כדי לסמן)" value={notDoneNoteDraft} onChange={(e) => setNotDoneNoteDraft(e.target.value)} />
            <button className="btn-ghost small danger" disabled={!notDoneNoteDraft.trim()} onClick={markNotDone}>סמן כלא בוצע</button>
          </div>
        )
      )}
...
const TABS = [
  ...
  { id: "open", label: "פעילות פתוחה", icon: AlertCircle },
...
.drag-handle { touch-action:none; cursor:grab; padding:6px; -webkit-user-select:none; user-select:none; }
.drag-row { transition: transform .15s ease, box-shadow .15s ease, opacity .15s ease; }
.drag-row.dragging { opacity:.9; transform:scale(1.02); box-shadow:0 8px 20px rgba(0,0,0,.18); border-radius:12px; z-index:5; position:relative; background:#fff; }
.drag-row.drag-over { box-shadow: 0 -3px 0 0 #2B4A5E inset; }
...
@media (max-width:900px) { .drag-handle { padding:10px; } .drag-handle svg { width:22px; height:22px; } }
