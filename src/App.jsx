          ...
          <span className="badge-group">
            ...
            {showDriverControls && (
              <span className="inline-driver-controls desktop-only">
                ...
                {onConfirmUpdate && <ConfirmStatusSelect task={task} onUpdate={onConfirmUpdate} />}
                {onConfirmUpdate && <ConfirmNoteInput task={task} onUpdate={onConfirmUpdate} />}
...
function ConfirmStatusSelect({ task, onUpdate }) {
  const setStatus = (val) => onUpdate({ ...task, confirmStatus: val });
  return (
    <select className="confirm-status-select" value={task.confirmStatus || ""} onChange={(e) => { e.stopPropagation(); setStatus(e.target.value); }} onClick={(e) => e.stopPropagation()} style={task.confirmStatus ? { color: CONFIRM_COLORS[task.confirmStatus], borderColor: CONFIRM_COLORS[task.confirmStatus], fontWeight: 700 } : undefined}>
      <option value="">— אישור —</option>
      {CONFIRM_OPTIONS.map((opt) => <option key={opt} value={opt}>{opt}</option>)}
    </select>
  );
}
function ConfirmNoteInput({ task, onUpdate }) {
  ...
  return (
    <input className="confirm-note-input" placeholder="הערה חופשית..." value={note} onChange={(e) => setNote(e.target.value)} onBlur={saveNote} onClick={(e) => e.stopPropagation()} />
  );
}
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      {!singleDriverMode && (
                        <div className="task-driver-inline mobile-only">
                          ...
                          <ConfirmStatusSelect task={t} onUpdate={(next) => updateTasks((prev) => prev.map((x) => x.id === t.id ? { ...x, ...next } : x))} />
                          <ConfirmNoteInput task={t} onUpdate={(next) => updateTasks((prev) => prev.map((x) => x.id === t.id ? { ...x, ...next } : x))} />
...
.confirm-status-select { border:1px solid #DED5BF; border-radius:999px; padding:3px 8px; font-size:11.5px; background:#FAFAF9; color:#3F3D38; max-width:110px; }
.confirm-note-row { display:flex; justify-content:flex-end; margin-top:4px; }
.confirm-note-input { border:1px solid #DED5BF; border-radius:999px; padding:4px 10px; font-size:11.5px; background:#FAFAF9; width:100%; max-width:220px; }
...
.group-driver-select { font-size:12.5px; font-weight:700; padding:5px 12px; border-radius:999px; border:none; background:#15161A; color:#fff; cursor:pointer; max-width:200px; }
