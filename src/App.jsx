...
const ZONES = [
  ...
  { id: "שרון", label: "שרון", color: "#0E8F6B" },
  { id: "ירושלים", label: "ירושלים", color: "#7B4FA0" },
...
const STATUS_FLOW_GENERAL = [
  ...
  { id: "בוטל", label: "בוטל", color: "#B23B3B" },
];
const STATUS_FLOW_CLEANING = [
  { id: "לפני ניקוי", label: "לפני ניקוי", color: "#6B6E76" },
...
const STATUS_FLOW_REPAIR = [
  { id: "לפני תיקון", label: "לפני תיקון", color: "#6B6E76" },
...
function CarpetIntakeTab({ tasks, updateTasks }) {
  ...
  return (
    <div>
      ...
      <textarea rows={8} value={text} onChange={(e) => setText(e.target.value)} placeholder={"4159\n4162\n4108"} />
...
function TransfersTab({ transfers, setTransfers, account }) {
  ...
  return (
    <div>
      ...
      <div className="filter-label">פריטים לבקשה</div>
      ...
      <div className="req-table">
        ...
        {draftRows.map((row, i) => (
          <div className="req-row" key={row.id}>
            <input value={row.sku} onChange={(e) => updateDraftRow(row.id, { sku: e.target.value })} onPaste={i === 0 ? handlePasteFromExcel : undefined} placeholder="10234" />
      ...
      <div className="transfer-list">
        {filtered.map((t) => (
          <div key={t.id} className="transfer-row">
            <input className="transfer-catalog-input" value={t.catalogNumber} onChange={(e) => updateItem(t.id, { catalogNumber: e.target.value })} placeholder="מק״ט" />
...
function CalendarTab({ tasks, updateTasks, selectedDate, onSelectDate, onStartAssign, config, account, driverNames }) {
  ...
  const [driverCountToday, setDriverCountToday] = useState(1);
  const [soloDriver, setSoloDriver] = useState("");
  const openTask = tasks.find((t) => t.id === openTaskId) || null;
  ...
  const assignSoloDriverToAll = () => {
    if (!soloDriver) return;
    const ids = allDayTasks.map((t) => t.id);
    updateTasks(tasks.map((t) => ids.includes(t.id) ? { ...t, driver: soloDriver, log: [...t.log, { ts: new Date().toISOString(), action: "נהג שויך (יום שלם)", note: soloDriver }] } : t));
  };
      ...
      <div className="ios-cal-agenda">
        ...
        {allDayTasks.length > 0 && (
          <div className="driver-today-box">
            <span className="muted small">כמה נהגים היום?</span>
            <div className="stepper">
              <button className="icon-btn" onClick={() => setDriverCountToday(Math.max(1, driverCountToday - 1))}>−</button>
              <b>{driverCountToday}</b>
              <button className="icon-btn" onClick={() => setDriverCountToday(driverCountToday + 1)}>+</button>
            </div>
            {driverCountToday === 1 ? (
              <>
                <select value={soloDriver} onChange={(e) => setSoloDriver(e.target.value)}>
                  <option value="">בחר נהג</option>
                  {driverNames && driverNames.map((n) => <option key={n} value={n}>{n}</option>)}
                </select>
                <button className="btn-primary small" onClick={assignSoloDriverToAll}><Car size={13}/> שייך לכל היום</button>
              </>
            ) : (
              <span className="muted small">מעל נהג אחד — שייכו נהג לכל אזור בנפרד למטה.</span>
            )}
          </div>
        )}
          ...
          return (
            <div key={zone.id} className="zone-block">
              ...
              <div className="driver-assign-row">
                <select value={driverDrafts[zone.id] || ""} onChange={(e) => setDriverDrafts({ ...driverDrafts, [zone.id]: e.target.value })}>
                  <option value="">— בחר נהג לכל האזור —</option>
                  {driverNames && driverNames.map((n) => <option key={n} value={n}>{n}</option>)}
                </select>
                <button className="btn-ghost small" onClick={() => assignDriverToZone(zone.id)}><Car size={13}/> שייך לכולם</button>
              </div>
...
.quick-assign-btn { display:flex; align-items:center; gap:3px; margin-right:auto; background:#F2F1EE; border:1px solid #E7E5E1; color:#33397A; border-radius:999px; padding:4px 10px; font-size:11px; font-weight:700; cursor:pointer; }
.driver-today-box { display:flex; align-items:center; gap:10px; flex-wrap:wrap; background:#fff; border:1px solid #E7E5E1; border-radius:11px; padding:10px 12px; margin-bottom:14px; }
.stepper { display:flex; align-items:center; gap:8px; font-family:'Rubik',sans-serif; font-weight:800; }
.stepper .icon-btn { border:1px solid #E7E5E1; border-radius:7px; width:26px; height:26px; }
...
@media (max-width: 560px) {
  ...
  .req-row { grid-template-columns: 1fr; background:#fff; border:1px solid #E7E5E1; border-radius:9px; padding:8px; }
