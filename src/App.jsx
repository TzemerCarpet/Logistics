...
function CalendarTab({ tasks, updateTasks, selectedDate, onSelectDate, onStartAssign, config, account, accounts, driverNames, getNextNumber }) {
  ...
  // מגאוקד את הכתובות ומסדר לפי "שכן קרוב" מהמחסן. נועל אחרי גרירה ידנית, ונפתח שוב כשמתווספת משימה חדשה לקבוצה.
  const autoSortSigRef = useRef({});
  useEffect(() => {
    let cancelled = false;
    (async () => {
      for (const group of driverGroups) {
        if (!group.key || group.tasks.length < 2) continue;
        const sig = group.tasks.map((t) => t.id).sort().join(",");
        const cacheKey = `${selectedDate}::${group.key}`;
        const allLocked = group.tasks.every((t) => t.routeLocked);
        if (allLocked) { autoSortSigRef.current[cacheKey] = sig; continue; }
        if (autoSortSigRef.current[cacheKey] === sig) continue; // כבר מוין אוטומטית לקבוצה הזו בדיוק — לא לחזור על עצמנו
        autoSortSigRef.current[cacheKey] = sig;
        ...
        if (!warehousePoint) continue; // אין מחסן מגואוקד — אין בסיס למיון, משאירים כמו שהיה
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedDate, JSON.stringify(driverGroups.map((g) => ({ k: g.key, ids: g.tasks.map((t) => t.id), locked: g.tasks.map((t) => t.routeLocked) })))]);
