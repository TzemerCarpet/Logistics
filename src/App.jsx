...
function TasksTab({ tasks, updateTasks, scope, getNextNumber, assignMode, onAssignPick, onCancelAssign, account, accounts, driverNames, quickAssignDate, onQuickAssign }) {
  ...
  ).sort((a,b) => new Date(b.createdAt) - new Date(a.createdAt)), [scoped, filterZone, filterStatus, assignMode]);
...
function SearchModal({ tasks, transfers, onClose, onOpenTask, onOpenTransfers, quickAssignDate, onQuickAssign }) {
  ...
  const taskResults = query ? tasks.filter(matchTask).sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).slice(0, 20) : [];
