    ...
    <div>
      ...
      {!branch ? (
        ...
        <>
          ...
          {filtered.length > 0 && (
            <div className="transfer-table-wrap desktop-only">
              <table className="transfer-table">
                <thead>
                  <tr>
              ...
              {filtered.map((t) => (
                <div key={t.id} className={"transfer-card" + (t.pending ? " row-pending" : t.rejected ? " row-rejected" : "") + (t.prepared ? " row-prepared" : "")}>
                  <div className="transfer-card-top">
                    ...
                    <button className="icon-btn danger" onClick={() => removeItem(t.id)}><Trash2 size={15}/></button>
                  </div>
                  {t.pending && <Chip color="#B8862F">ממתין לאישור</Chip>}
                  {t.rejected && <div className="reject-note">נדחה ע״י מחסן: {t.rejectionReason || "—"}</div>}
                  <div className="transfer-card-grid">
                    <Field label="מק״ט"><input inputMode="numeric" pattern="[0-9]*" value={t.catalogNumber} onChange={(e) => updateItem(t.id, { catalogNumber: e.target.value })} placeholder="מק״ט" /></Field>
                    <Field label="מידה"><input value={t.size || ""} onChange={(e) => updateItem(t.id, { size: e.target.value })} placeholder="מידה" /></Field>
                  </div>
                  <Field label="הערות"><input value={t.notes || ""} onChange={(e) => updateItem(t.id, { notes: e.target.value })} placeholder="הערות" /></Field>
                  <Field label="סטטוס"><select value={t.status} onChange={(e) => updateItem(t.id, { status: e.target.value })} style={{ color: TRANSFER_STATUS.find(s=>s.id===t.status)?.color }}>{TRANSFER_STATUS.map((s) => <option key={s.id} value={s.id}>{s.label}</option>)}</select></Field>
                </div>
              ))}
            </div>
          )}
                ...
                {gTasks.map((t, i) => (
                  <div key={t.id} className={"drag-row" + (draggingId === t.id ? " dragging" : "") + (overId === t.id && draggingId !== t.id ? " drag-over" : "")} data-group={group.key} data-taskid={t.id}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div className="confirm-note-row" onClick={(e) => e.stopPropagation()}>
...
.transfer-card-list { display:flex; flex-direction:column; gap:10px; }
.transfer-card { background:#fff; border:1px solid #DED5BF; border-radius:12px; padding:12px; }
...
.transfer-card-top { display:flex; align-items:center; gap:8px; margin-bottom:8px; }
.transfer-card-name { flex:1; font-weight:700; font-size:15px; border:none; background:transparent; padding:4px 0; }
.transfer-card-grid { display:grid; grid-template-columns:1fr 1fr; gap:8px; }
