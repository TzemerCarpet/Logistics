...
function canSeeTask(task, account) {
  ...
  if (task.rejected) return task.submittedBy === account.name;
...
function canSeeTransfer(t, account) {
  if (!account) return false;
  if (t.rejected) return t.submittedBy === account.name;
  if (t.pending) return t.submittedBy === account.name;
  return true;
}
...
function TasksTab({ tasks, updateTasks, scope, getNextNumber, assignMode, onAssignPick, onCancelAssign, account, driverNames, quickAssignDate, onQuickAssign }) {
  ...
  const openCount = scoped.filter((t) => (filterZone.length === 0 || filterZone.includes(t.zone)) && !["הסתיים", "בוטל"].includes(t.status) && !t.pending && !t.rejected).length;
  ...
  return (
    <div>
      ...
      <div className="task-list">{filtered.map((t) => <TaskCard key={t.id} task={t} onOpen={() => assignMode ? onAssignPick(t.id) : setOpenTaskId(t.id)} quickAssignDate={!assignMode ? quickAssignDate : null} onQuickAssign={account?.role === "admin" ? onQuickAssign : null} />)}</div>
...
function TransfersTab({ transfers, setTransfers, account }) {
  ...
  const lockedBranch = account?.role === "branch" && account?.branchId ? account.branchId : null;
  ...
  const visible = transfers.filter((t) => canSeeTransfer(t, account));
...
function AccountsAdmin({ accounts, setAccounts }) {
  ...
  const save = () => {
    ...
    if (editing.role === "branch" && !editing.branchId) return;
        ...
        {accounts.map((a) => (
          <div key={a.id} className="account-row">
            ...
            <div style={{ flex: 1 }}>
              ...
              <div className="driver-meta">{a.role === "admin" ? "גישה מלאה" : a.role === "manager" ? "רואה הכל, אך תוספותיו ממתינות לאישור" : a.role === "driver" ? "רואה רק את המשימות המשויכות אליו" : a.canViewAllTasks ? "רואה את כל המשימות" : "רואה רק את מה שהוא שלח"}{a.role === "branch" && (a.branchId ? ` · סניף: ${branchLabel(a.branchId)}` : " · לא משויך לסניף")}</div>
            </div>
...
export default function App() {
  ...
  const counts = {
    ...
    transfers: transfers.filter((t) => canSeeTransfer(t, account) && t.status !== "הועבר").length,
  ...
  return (
    <div dir="rtl" className="app-root">
      ...
      <div className={"shell" + (tab === "calc" ? " no-rail" : "")}>
        ...
        <main className="main">
          ...
          {tab === "tasks" ? <TasksTab key="tasks" tasks={tasks} updateTasks={updateTasks} scope="general" assignMode={assignMode} onAssignPick={assignPick} onCancelAssign={cancelAssign} account={account} driverNames={driverNames} quickAssignDate={scheduleDate} onQuickAssign={quickAssignToSchedule} /> :
           ...
           tab === "calendar" && (isAdmin || account?.canViewAllTasks) ? <CalendarTab tasks={tasks} updateTasks={updateTasks} selectedDate={scheduleDate} onSelectDate={setScheduleDate} onStartAssign={startAssign} config={config} account={account} driverNames={driverNames} getNextNumber={getNextNumber} /> :
      ...
      {searchOpen && <SearchModal tasks={visibleTasks} transfers={transfers.filter((t) => canSeeTransfer(t, account))} onClose={() => setSearchOpen(false)} onOpenTask={openFromSearch} onOpenTransfers={openTransfersFromSearch} />}
