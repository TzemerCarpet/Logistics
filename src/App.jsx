      ...
      <div className="transfer-list">
        {pendingTransfers.map((tr) => (
          <div key={tr.id} className="approval-row transfer-approval-row">
            ...
            <div className="muted small transfer-row-details">
              {tr.size && <span>מידה: {tr.size}</span>}
              {tr.notes && <span>הערות: {tr.notes}</span>}
              <span>נשלח ע״י: {tr.submittedBy || "—"}</span>
            </div>
...
.transfer-row-details { display:flex; gap:14px; flex-wrap:wrap; margin:4px 0 8px; }
