...
function CalendarTab({ tasks, updateTasks, selectedDate, onSelectDate, onStartAssign, config, account, accounts, driverNames, getNextNumber }) {
  ...
  const [draggingId, setDraggingId] = useState(null); // רק לצורך הדגשה ויזואלית
  ...
  const onHandlePointerMove = (e) => {
    ...
    if (targetGroup !== dragRef.current.groupKey || targetId === dragRef.current.taskId) return;
    const group = driverGroups.find((g) => g.key === dragRef.current.groupKey);
    if (!group) return;
    const arr = [...group.tasks];
    const fromIdx = arr.findIndex((t) => t.id === dragRef.current.taskId);
  ...
  const onHandlePointerUp = () => { dragRef.current = null; setDraggingId(null); };
          ...
          return (
            <div key={group.key || "__unassigned__"} className="zone-block">
              ...
              <div className="task-list route-thread">
                {gTasks.map((t, i) => (
                  <div key={t.id} className={"drag-row" + (draggingId === t.id ? " dragging" : "")} data-group={group.key} data-taskid={t.id}>
