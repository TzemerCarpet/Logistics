...
function ConfirmStatusRow({ task, onUpdate }) {
  const [note, setNote] = useState(task.confirmNote || "");
  useEffect(() => { setNote(task.confirmNote || ""); }, [task.id]);
  const setStatus = (opt) => onUpdate({ ...task, confirmStatus: task.confirmStatus === opt ? "" : opt });
  const saveNote = () => { if (note !== (task.confirmNote || "")) onUpdate({ ...task, confirmNote: note }); };
  return (
    <div className="confirm-row pill-style" onClick={(e) => e.stopPropagation()}>
      {CONFIRM_OPTIONS.map((opt) => (
        <button key={opt} type="button" className={"confirm-btn" + (task.confirmStatus === opt ? " active" : "")} onClick={() => setStatus(opt)}>{opt}</button>
      ))}
      <input className="confirm-note-input" placeholder="הערה חופשית..." value={note} onChange={(e) => setNote(e.target.value)} onBlur={saveNote} />
    </div>
  );
}
                ...
                {gTasks.map((t, i) => (
                  <div key={t.id} className={"drag-row" + (draggingId === t.id ? " dragging" : "") + (overId === t.id && draggingId !== t.id ? " drag-over" : "")} data-group={group.key} data-taskid={t.id}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <ConfirmStatusRow task={t} onUpdate={(next) => updateTasks(tasks.map((x) => x.id === t.id ? next : x))} />
                    </div>
                  </div>
                ))}
              </div>
...
export default function App() {
  ...
  // רענון אוטומטי של הדף כל 5 דקות — רק אם אין חלון פתוח ואף אחד לא באמצע כתיבה/עריכה
  useEffect(() => {
    let idleChecks = 0;
    const IDLE_TARGET = 5; // 5 בדיקות של דקה = 5 דקות
    const interval = setInterval(() => {
      const active = document.activeElement;
      const isTyping = active && ["INPUT", "TEXTAREA", "SELECT"].includes(active.tagName);
      const hasModal = !!document.querySelector(".modal-backdrop");
      if (isTyping || hasModal) { idleChecks = 0; return; }
      idleChecks += 1;
      if (idleChecks >= IDLE_TARGET) window.location.reload();
    }, 60 * 1000);
  ...
  // סנכרון רקע — כל 15 שניות שולפים נתונים טריים מהשרת בלי לרענן את הדף,
  // כדי לראות שינויים שאחרים עשו כמעט מיד. מדלגים אם יש חלון פתוח או שמישהו כותב עכשיו.
  useEffect(() => {
    if (!ready) return;
    const interval = setInterval(async () => {
      ...
      const isTyping = active && ["INPUT", "TEXTAREA", "SELECT"].includes(active.tagName);
      const hasModal = !!document.querySelector(".modal-backdrop");
      if (isTyping || hasModal) return;
      try {
        const [t, tr] = await Promise.all([loadTasks(), loadTransfers()]);
        setTasks(t);
        setTransfers(tr);
      } catch (e) { console.error(e); }
    }, 15 * 1000);
...
.confirm-row { display:flex; gap:6px; flex-wrap:wrap; align-items:center; margin-top:6px; padding:8px; background:#F7F5EC; border-radius:8px; }
.confirm-btn { font-size:12.5px; font-weight:700; padding:6px 10px; border-radius:16px; border:1px solid #C9C4B4; background:#fff; color:#5A5648; cursor:pointer; white-space:nowrap; }
.confirm-btn.active { background:#2B4A5E; border-color:#2B4A5E; color:#fff; }
.confirm-note-input { flex:1; min-width:120px; font-size:12.5px; padding:6px 10px; border-radius:16px; border:1px solid #C9C4B4; background:#fff; }
.confirm-row.pill-style { display:flex; align-items:center; justify-content:flex-end; gap:5px; margin-top:5px; flex-wrap:wrap; background:none; padding:0; }
.confirm-row.pill-style .confirm-btn { border:1px solid #DED5BF; border-radius:999px; padding:3px 10px; font-size:11px; background:#FAFAF9; color:#3F3D38; }
.confirm-row.pill-style .confirm-btn.active { background:#2B4A5E; border-color:#2B4A5E; color:#fff; }
.confirm-row.pill-style .confirm-note-input { border:1px solid #DED5BF; border-radius:999px; padding:3px 10px; font-size:11px; background:#FAFAF9; max-width:150px; }
