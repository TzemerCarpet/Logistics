...
function canSeeTransfer(t, account) {
  ...
  if (t.pending) return isAdmin || t.submittedBy === account.name;
  return true;
}
    ...
    <div>
      ...
      <div className="task-list">
        {rejectedTasks.map((t) => (
          <div key={t.id} className="approval-row">
            <TaskCard task={t} onOpen={() => {}} />
            <div className="muted small" style={{ margin: "4px 0 0" }}>נשלח ע״י: {t.submittedBy} · סיבת הדחייה: {t.rejectionReason || "—"}</div>
          </div>
        ))}
      </div>
      ...
      <div className="transfer-list">
        {rejectedTransfers.map((tr) => (
          <div key={tr.id} className="approval-row transfer-approval-row">
            ...
            <div className="muted small" style={{ margin: "4px 0 0" }}>נשלח ע״י: {tr.submittedBy || "—"} · סיבת הדחייה: {tr.rejectionReason || "—"}</div>
          </div>
        ))}
      </div>
    </div>
...
.transfer-approval-row .chip { margin-inline-start: auto; }
