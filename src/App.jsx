...
function TaskCard({ task, onOpen, quickAssignDate, onQuickAssign }) {
    ...
    <div className="task-card" style={{ "--zone-color": zoneColor }} onClick={onOpen}>
      ...
      <div className="body-col">
        ...
        <div className="facts">
          ...
          <span className="fact"><span className="lbl">אזור</span><b style={{ color: zoneColor }}>{task.zone}</b></span>
          {firstCarpet && <span className="fact"><span className="lbl">{tMeta.label}</span><b>{firstCarpet.number ? `#${firstCarpet.number}` : "ללא מספר"}{firstSize ? ` · ${firstSize}` : ""}{task.carpets.length > 1 ? ` +${task.carpets.length - 1}` : ""}</b></span>}
          ...
          {task.receivedDate && <span className="fact"><span className="lbl">התקבל</span><b>{task.receivedDate}</b></span>}
          {!isGeneral && !isBranch && task.inProgressDate && <span className="fact"><span className="lbl">בטיפול מ</span><b>{task.inProgressDate}</b></span>}
          {task.scheduledDate && <span className="fact"><span className="lbl">מתוזמן</span><b>{task.scheduledDate}</b></span>}
        ...
        {quickAssignDate && onQuickAssign && (
          <button className="quick-assign-btn" onClick={(e) => { e.stopPropagation(); onQuickAssign(task.id); }}>
            <Plus size={11}/> {task.scheduledDate === quickAssignDate ? `כבר בלו״ז ${quickAssignDate}` : `הוסף ללו״ז ${quickAssignDate}`}
          </button>
        )}
      </div>
    </div>
...
function CarpetIntakeTab({ tasks, updateTasks }) {
  ...
  // מספרי שטיחים הם תמיד 4 ספרות — ברגע שהוקלדו 4 ספרות עוברים אוטומטית לשורה הבאה.
  const handleTextChange = (e) => {
    const raw = e.target.value;
    const digitsOnly = raw.replace(/[^0-9]/g, "");
    const groups = digitsOnly.match(/.{1,4}/g) || [];
    let formatted = groups.join("\n");
    if (digitsOnly.length > 0 && digitsOnly.length % 4 === 0) formatted += "\n";
    setText(formatted);
  };
  ...
  const process = () => {
    ...
    const next = tasks.map((t) => {
      ...
      const hasMatch = (t.carpets || []).some((c) => c.number && numbers.includes(String(c.number)));
      if (!hasMatch) return t;
      (t.carpets || []).forEach((c) => { if (c.number && numbers.includes(String(c.number))) matchedNumbers.add(String(c.number)); });
    ...
    const notFound = numbers.filter((n) => !matchedNumbers.has(n));
  ...
  return (
    <div>
      <p className="muted small" style={{ marginBottom: 12 }}>הדביקו/הקלידו מספרי הזמנה שהתקבלו במחסן (שורה לכל מספר, או מופרדים בפסיק). כל משימת ניקוי/תיקון עם מספר תואם תעודכן אוטומטית ל"לפני אספקה" — אפשר לעשות זאת גם מהטלפון וגם מהמחשב.</p>
      <textarea rows={8} inputMode="numeric" value={text} onChange={handleTextChange} placeholder={"4159\n4162\n4108"} />
      <button className="btn-primary" style={{ marginTop: 10 }} onClick={process}><CheckCircle2 size={16}/> אשר קליטה</button>

      {result && (
        <>
          <div className="error-box" style={{ background: result.notFound.length ? "#FFF4E0" : "#E8F5EC", color: "#15161A", borderColor: "transparent", marginTop: 16 }}>
            עודכנו {result.matchedTasks.length} משימות.
            {result.notFound.length > 0 && <> לא נמצאו: {result.notFound.join(", ")}</>}
          </div>
...
function TasksTab({ tasks, updateTasks, scope, getNextNumber, assignMode, onAssignPick, onCancelAssign, account, driverNames, quickAssignDate, onQuickAssign }) {
  ...
  return (
    <div>
      ...
      <div className="filter-label">אזור <span className="muted small">(ניתן לבחור כמה)</span></div>
      <PillGroup options={ZONES} value={filterZone} onChange={setFilterZone} size="sm" multi />

      <div className="filter-label">סטטוס <span className="muted small">(ניתן לבחור כמה)</span></div>
      ...
      {!branch ? (
        ...
        <>
          ...
          <div className="req-table">
            <div className="req-head"><span>מק״ט</span><span>שם</span><span>מידה</span><span>הערות</span><span/></div>
            {draftRows.map((row, i) => (
              <div className="req-row" key={row.id}>
                <input value={row.sku} inputMode="numeric" pattern="[0-9]*" onChange={(e) => updateDraftRow(row.id, { sku: e.target.value })} onPaste={i === 0 ? handlePasteFromExcel : undefined} placeholder="10234" />
                <input value={row.name} onChange={(e) => updateDraftRow(row.id, { name: e.target.value })} placeholder="שטיח פרסי כחול" />
                <input value={row.size} onChange={(e) => updateDraftRow(row.id, { size: e.target.value })} placeholder="200×300" />
                <input value={row.notes} onChange={(e) => updateDraftRow(row.id, { notes: e.target.value })} placeholder="הערה (לא חובה)" />
                <button className="icon-btn danger" onClick={() => removeDraftRow(row.id)}><Trash2 size={14}/></button>
              </div>
            ))}
          </div>
          ...
          <div className="transfer-list">
            {filtered.map((t) => (
              <div key={t.id} className="transfer-row">
                <input className="transfer-catalog-input" inputMode="numeric" pattern="[0-9]*" value={t.catalogNumber} onChange={(e) => updateItem(t.id, { catalogNumber: e.target.value })} placeholder="מק״ט" />
                ...
                <input className="transfer-size-input" value={t.size || ""} onChange={(e) => updateItem(t.id, { size: e.target.value })} placeholder="מידה" />
                <input className="transfer-size-input" value={t.notes || ""} onChange={(e) => updateItem(t.id, { notes: e.target.value })} placeholder="הערות" />
...
function CalendarTab({ tasks, updateTasks, selectedDate, onSelectDate, onStartAssign, config, account, driverNames }) {
  ...
  return (
    <div>
            ...
            <div key={group.key || "__unassigned__"} className="zone-block">
              ...
              {!singleDriverMode && (
                <div className="driver-assign-row">
                  <select value={driverDrafts[group.key] || ""} onChange={(e) => setDriverDrafts({ ...driverDrafts, [group.key]: e.target.value })}>
                    <option value="">— בחר נהג לכל הקבוצה —</option>
                    {driverNames && driverNames.map((n) => <option key={n} value={n}>{n}</option>)}
                  </select>
                  <button className="btn-ghost small" onClick={() => assignDriverToGroup(group.key, driverDrafts[group.key])}><Car size={13}/> שייך לכולם</button>
                </div>
              )}
      ...
      <div className="print-manifest">
        ...
        {driverGroups.filter((g) => g.tasks.length > 0).map((group) => (
          <div key={group.key || "__unassigned__"} className="print-zone">
            <h3>{group.key === "" ? "ללא שיוך" : `נהג: ${group.driver}`} ({group.tasks.length} עצירות)</h3>
            {group.tasks.map((t, i) => {
              const isG = t.type === "general", isB = t.type === "branch", isC = !isG && !isB;
              return (
                <div key={t.id} className="print-stop">
                  <div className="print-stop-head">{i + 1}. {isG || isB ? t.title : t.client.name} <span className="print-type">· {typeMeta(t.type).label} · {t.zone}</span></div>
      ...
      {openTask && <TaskDetail task={openTask} onClose={() => setOpenTaskId(null)} onUpdate={(t) => updateTasks(tasks.map((x) => x.id === t.id ? t : x))} driverNames={driverNames} />}
    </div>
  );
}
    ...
    <div dir="rtl" className="app-root">
      ...
      <div className={"shell" + (tab === "calc" ? " no-rail" : "")}>
        ...
        <main className="main">
          ...
          {tab === "tasks" ? <TasksTab tasks={tasks} updateTasks={updateTasks} scope="general" assignMode={assignMode} onAssignPick={assignPick} onCancelAssign={cancelAssign} account={account} driverNames={driverNames} quickAssignDate={scheduleDate} onQuickAssign={quickAssignToSchedule} /> :
           tab === "ops" ? <TasksTab tasks={tasks} updateTasks={updateTasks} scope="ops" getNextNumber={getNextNumber} assignMode={assignMode} onAssignPick={assignPick} onCancelAssign={cancelAssign} account={account} driverNames={driverNames} quickAssignDate={scheduleDate} onQuickAssign={quickAssignToSchedule} /> :
           ...
           tab === "calendar" ? <CalendarTab tasks={tasks} updateTasks={updateTasks} selectedDate={scheduleDate} onSelectDate={setScheduleDate} onStartAssign={startAssign} config={config} account={account} driverNames={driverNames} /> :
...
/* sidebar */
.sidebar { background:#fff; border-left:1px solid #E4E1D9; padding:20px 12px; display:flex; flex-direction:column; position:sticky; top:0; height:100vh; overflow-y:auto; }
...
.sb-spacer { flex:1; }
.sb-item.settings { border-top:1px solid #E4E1D9; margin-top:8px; padding-top:14px; }
.sb-item.search-item { border:1px solid #E4E1D9; margin-bottom:14px; background:#fff; }
...
.driver-daynav { display:flex; align-items:center; justify-content:center; gap:14px; background:#fff; border:1px solid #E4E1D9; border-radius:12px; padding:10px; margin-bottom:16px; font-weight:700; font-family:'Rubik',sans-serif; font-size:14px; }
.driver-progress { background:#fff; border:1px solid #E4E1D9; border-radius:12px; padding:12px 14px; margin-bottom:18px; }
...
.driver-card { background:#fff; border:1px solid #E4E1D9; border-radius:14px; padding:14px 16px; margin-bottom:12px; }
...
.driver-action-btn.call { background:#F2F1EE; color:#15161A; border:1px solid #E4E1D9; }
...
.login-card { background:#fff; border:1px solid #E4E1D9; border-radius:16px; padding:32px 28px; width:100%; max-width:340px; }
...
.account-row { display:flex; align-items:center; gap:10px; background:#fff; border:1px solid #E4E1D9; border-radius:11px; padding:10px 12px; }
.approval-row { background:#fff; border:1px solid #E4E1D9; border-radius:14px; padding:12px; }
...
.stat-card { flex:1; background:#fff; border:1px solid #E4E1D9; border-radius:11px; padding:13px 15px; }
...
/* right rail */
.rail { border-right:1px solid #E4E1D9; padding:24px 20px; background:#fff; position:sticky; top:0; height:100vh; overflow-y:auto; }
...
.mini-cal-grid .day:hover { background:#E4E1D9; }
...
.driver-row { display:flex; align-items:center; gap:10px; padding:10px 12px; border:1px solid #E4E1D9; border-radius:11px; }
...
@media (max-width: 900px) {
  ...
  .mobile-topbar { display:flex; align-items:center; gap:12px; position:sticky; top:0; z-index:25; background:#fff; padding:14px 16px; border-bottom:1px solid #E4E1D9; }
  ...
  .sidebar {
    ...
    flex-direction:column; overflow-y:auto; border-left:1px solid #E4E1D9; border-bottom:none;
...
.pill { display:flex; align-items:center; gap:6px; background:#fff; border:1.5px solid #E4E1D9; color:#8A8677; padding:9px 15px; border-radius:10px; font-weight:700; cursor:pointer; font-family:'Assistant',sans-serif; font-size: 13.5px; }
...
.filters select { background:#fff; border:1px solid #E4E1D9; color:#15161A; border-radius:8px; padding:7px 10px; font-family:'Assistant',sans-serif; font-size:13px; }
...
.task-card { position:relative; display:flex; align-items:stretch; gap:14px; background:#fff; border:1px solid #E4E1D9; border-radius: 13px; padding: 14px 16px; cursor:pointer; transition: border-color .15s ease, box-shadow .15s ease, transform .15s ease; overflow:hidden; }
...
.time-col .time { font-family:'Rubik',sans-serif; font-size:15px; font-weight:800; color:#15161A; font-variant-numeric:tabular-nums; line-height:1.1; text-align:center; }
.time-col .time.empty { color:#C9C6BC; font-size:13px; font-weight:700; }
.time-col .time.range { display:flex; flex-direction:column; align-items:center; font-size:12px; font-weight:800; line-height:1.2; }
...
.fact b { color:#15161A; font-weight:700; font-variant-numeric:tabular-nums; }
...
.modal { background:#FFFFFF; border:1px solid #E4E1D9; border-radius:16px; width:100%; max-width:640px; max-height: 90vh; display:flex; flex-direction:column; overflow:hidden; }
.modal-head { display:flex; justify-content:space-between; align-items:center; padding:16px 18px; border-bottom:1px solid #E4E1D9; background:#fff; }
.modal-head h2 { font-family:'Rubik',sans-serif; font-size:18px; margin:0; }
.modal-body { padding: 16px 18px; overflow-y:auto; }
.modal-foot { display:flex; justify-content:flex-end; gap:10px; padding: 14px 18px; border-top:1px solid #E4E1D9; background:#fff; }

.icon-btn { background:transparent; border:none; color:#8A8677; cursor:pointer; padding:6px; border-radius:8px; }
.icon-btn:hover { background:#E4E1D9; color:#15161A; }
...
.field input, .field select, .field textarea, select, input[type=text], input[type=number], input[type=date], input[type=password], textarea {
  background:#fff; border:1px solid #E4E1D9; color:#15161A; border-radius:8px; padding:9px 10px; font-family:'Assistant',sans-serif; font-size:14px; width:100%; resize: vertical;
}
.field input:focus, .field select:focus, .field textarea:focus { outline:2px solid #33397A55; border-color:#33397A; }
.task-description { font-size:13.5px; line-height:1.5; color:#33343A; background:#fff; border:1px solid #E4E1D9; border-radius:8px; padding:10px 12px; }

.carpet-row { background:#fff; border:1px solid #E4E1D9; border-radius:12px; padding:12px; margin-bottom:10px; }
...
.quick-assign-btn { display:flex; align-items:center; gap:3px; margin-right:auto; background:#F2F1EE; border:1px solid #E4E1D9; color:#33397A; border-radius:999px; padding:4px 10px; font-size:11px; font-weight:700; cursor:pointer; }
.driver-today-box { display:flex; align-items:center; gap:10px; flex-wrap:wrap; background:#fff; border:1px solid #E4E1D9; border-radius:11px; padding:10px 12px; margin-bottom:14px; }
.stepper { display:flex; align-items:center; gap:8px; font-family:'Rubik',sans-serif; font-weight:800; }
.stepper .icon-btn { border:1px solid #E4E1D9; border-radius:7px; width:26px; height:26px; }
...
.extra-pill { display:flex; align-items:center; gap:6px; background:#FAFAF9; border:1px solid #E4E1D9; border-radius:999px; padding:6px 12px; font-size:12.5px; cursor:pointer; }
...
.carpet-summary-block { background:#fff; border:1px solid #E4E1D9; border-radius:10px; padding:10px 12px; margin-bottom:8px; }
...
.photo-thumb { position:relative; width:64px; height:64px; border-radius:8px; overflow:hidden; border:1px solid #E4E1D9; }
...
.detail-actions-float { position:fixed; bottom:18px; left:50%; transform:translateX(-50%); display:flex; gap:8px; z-index:60; background:#fff; border:1px solid #E4E1D9; padding:8px; border-radius:12px; box-shadow: 0 4px 16px #15161A20; }

.paste-preview { display:flex; align-items:center; gap:6px; font-size:12.5px; color:#8A8677; background:#fff; border:1px solid #E4E1D9; border-radius:8px; padding:8px 10px; margin-top:4px; }
.req-table { display:flex; flex-direction:column; gap:6px; margin-bottom:10px; }
.req-head, .req-row { display:grid; grid-template-columns: 1fr 1.6fr 0.8fr 1.4fr 28px; gap:8px; align-items:center; }
.req-head span { font-size:11px; font-weight:700; color:#8A8677; padding: 0 2px; }
.req-row input { font-size:13px; }

.transfer-list { display:flex; flex-direction:column; gap:8px; }
.transfer-row { display:flex; align-items:center; gap:10px; background:#fff; border:1px solid #E4E1D9; border-radius:10px; padding:10px 14px; flex-wrap:wrap; }
...
.task-driver-inline input { border:1px solid #E4E1D9; border-radius:6px; padding:4px 8px; font-size:12.5px; background:#FAFAF9; }
.remove-day-btn { display:flex; align-items:center; gap:3px; background:transparent; border:1px solid #E4E1D9; color:#8A8677; border-radius:6px; padding:4px 8px; font-size:11.5px; font-weight:600; cursor:pointer; }
...
.pick-row { display:flex; align-items:center; gap:10px; background:#fff; border:1px solid #E4E1D9; border-radius:10px; padding:10px 12px; margin-bottom:8px; cursor:pointer; }
...
.print-controls { display:flex; align-items:center; gap:10px; flex-wrap:wrap; margin-bottom:14px; background:#fff; border:1px solid #E4E1D9; border-radius:11px; padding:10px 12px; }
...
.print-type { font-weight:400; color:#8A8677; font-size:12px; }
...
@media (max-width: 560px) {
  ...
  .req-row { grid-template-columns: 1fr; background:#fff; border:1px solid #E4E1D9; border-radius:9px; padding:8px; }
