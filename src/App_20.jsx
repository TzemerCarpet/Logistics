    ...
    <div className="modal-backdrop">
      <div className="modal">
        ...
        <div className="modal-body">
          ...
          {showExtraZones ? (
            <Field label="אזורים נוספים (לא חובה)"><PillGroup options={ZONES.filter((z) => z.id !== zone)} value={extraZones} onChange={setExtraZones} multi /></Field>
          ) : (
...
function TransfersTab({ transfers, setTransfers, account, accounts }) {
...
function DriverTaskCard({ task, stopNumber, onUpdate }) {
  ...
  return (
    <div className={"driver-card" + (isTaskDone(task) ? " done" : "")}>
      ...
      {task.client?.address && <div className="driver-card-addr"><MapPin size={13}/> {task.client.address}{aptFloorSuffix(task.client)}</div>}
      ...
      <div className={"shell" + (tab === "calc" ? " no-rail" : "")}>
        ...
        <main className="main">
          ...
          {tab === "tasks" ? <TasksTab key="tasks" tasks={tasks} updateTasks={updateTasks} scope="general" assignMode={assignMode} onAssignPick={assignPick} onCancelAssign={cancelAssign} account={account} accounts={accounts} driverNames={driverNames} quickAssignDate={scheduleDate} onQuickAssign={quickAssignToSchedule} /> :
           ...
           tab === "transfers" ? <TransfersTab transfers={transfers} setTransfers={setTransfers} account={account} accounts={accounts} /> :
...
.driver-note-display { font-size:13px; font-weight:600; color:#7A4A1E; line-height:1.5; margin:4px 0 9px; padding:7px 10px; background:#FBF0DD; border-right:3px solid #C77A2A; border-radius:6px; }
