...
import {
  ...
  Wrench, Sparkles, MessageCircle, Building2, GripVertical, Search, ChevronUp, ChevronDown, CheckCircle2, Menu, Eye, EyeOff
} from "lucide-react";
  ...
  return (
    <div className="confirm-block" ref={boxRef} onClick={(e) => e.stopPropagation()}>
      <button type="button" className="confirm-status-btn" onClick={() => setOpen((v) => !v)} style={activeColor ? { color: activeColor, borderColor: activeColor } : undefined}>
        {task.confirmStatus || "סטטוס"}{task.confirmNote ? " 📝" : ""}
      </button>
      {open && (
        <div className="confirm-popover">
          ...
          <input className="confirm-popover-note" autoFocus placeholder="הערה חופשית..." value={note} onChange={(e) => setNote(e.target.value)} onKeyDown={(e) => e.key === "Enter" && setOpen(false)} />
        </div>
      )}
    </div>
  );
